  ...
  return (
    <div className={`flex h-screen w-full flex-col transition-colors duration-500 ${theme === 'dark' ? 'bg-[#0F0A1A]' : 'bg-background-light'} overflow-hidden font-display`}>
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        <aside className={`w-full lg:w-[480px] border-r p-8 flex flex-col gap-6 shadow-2xl z-10 transition-colors overflow-y-auto ${theme === 'dark' ? 'bg-[#1A1030] border-[#251848]' : 'bg-white border-primary/10'}`}>
          <div>
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/10 text-primary w-fit text-base font-black uppercase tracking-widest mb-3">Module 04</div>
            <h1 className={`text-6xl font-black ${theme === 'dark' ? 'text-[#E8DCFF]' : 'text-[#1A1A1A]'}`}>{language === 'EN' ? 'Dynamics' : 'ความดัง-เบา'}</h1>
            <p className="text-gray-500 mt-2 text-lg">{language === 'EN' ? 'Variation in loudness.' : 'ความแปรผันของความดังระหว่างโน้ต'}</p>
          </div>
          <div className="flex-1 flex flex-col items-center justify-center py-4 gap-8">
            <div className="flex w-full justify-center gap-12 items-center">
               <div className={`flex flex-col justify-between h-[300px] text-3xl font-black italic transition-opacity ${theme === 'dark' ? 'text-white/20' : 'text-slate-200'}`}>
               ...
               <div ref={sliderRef} onMouseDown={onMouseDown} onTouchMove={(e) => handleSliderInteraction(e.touches[0].clientY)} className={`relative h-[300px] w-24 rounded-[2rem] border-4 cursor-pointer overflow-hidden group shadow-inner transition-colors ${theme === 'dark' ? 'bg-[#0F0A1A] border-[#251848]' : 'bg-slate-50 border-slate-100'}`}>
                  ...
                  <div className="absolute left-1/2 -translate-x-1/2 flex flex-col items-center transition-all duration-75 pointer-events-none" style={{ bottom: `calc(${volume}% - 28px)` }}>
                     <div className="size-14 rounded-full bg-white shadow-xl flex items-center justify-center border-2 border-primary"><span className="material-symbols-outlined text-primary text-2xl">drag_handle</span></div>
                  </div>
               </div>
            </div>
          </div>
          <div className={`p-8 rounded-[2rem] border transition-colors ${theme === 'dark' ? 'bg-[#0F0A1A] border-[#251848]' : 'bg-slate-50 border-slate-200'}`}>
             <p className="text-base font-bold text-gray-400 mb-3 uppercase tracking-widest">{t.volume[language]}</p>
             <div className="flex items-center justify-between">
                <span className="text-5xl font-black text-[#FFDE59]">{intensity}</span>
                <span className={`text-xl font-black px-5 py-3 rounded-full border shadow-sm ${theme === 'dark' ? 'bg-[#251848] text-[#E8DCFF] border-[#251848]' : 'bg-white text-slate-700'}`}>{Math.round(volume)}%</span>
             </div>
          </div>
        </aside>
        ...
        <section className={`flex-1 flex flex-col items-center justify-center stage-grid relative overflow-hidden transition-colors ${theme === 'dark' ? 'bg-[#0F0A1A]' : 'bg-background-light'}`}>
          <div style={{ transform: `scale(${starScale}) rotate(${volume}deg)`, transition: 'transform 0.1s ease-out' }} className="relative">
            <div className="absolute inset-0 bg-primary blur-[120px] rounded-full opacity-30 animate-pulse" style={{ transform: `scale(${1 + volume/100})` }}></div>
            <svg width="600" height="600" viewBox="0 0 100 100" className="fill-primary drop-shadow-2xl relative z-10">
              <path d="M50 5 L63 40 L98 40 L70 62 L80 95 L50 75 L20 95 L30 62 L2 40 L37 40 Z" />
            </svg>
          </div>
          <div className={`absolute bottom-12 px-12 py-6 rounded-3xl border shadow-2xl flex flex-col items-center gap-2 text-center max-w-lg transition-colors ${theme === 'dark' ? 'bg-[#1A1030]/80 border-[#251848] text-[#E8DCFF]' : 'bg-white/80 border-primary/10 text-[#1A1A1A]'}`}>
             <span className="text-lg font-black uppercase tracking-[0.3em] text-primary">{t.visualImpact[language]}</span>
