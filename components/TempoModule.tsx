        ...
        <div className="flex-1 flex flex-col p-10 gap-8 overflow-y-auto">
          <div className="flex flex-col gap-2 relative">
            <div className="relative z-10 block">
              ...
              <h1 className={`text-6xl font-black ${theme === 'dark' ? 'text-[#E8DCFF]' : 'text-[#1A1A1A]'}`}>
            ...
            <p className={`text-xl max-w-2xl ${theme === 'dark' ? 'text-[#9B7EC8]' : 'text-gray-500'}`}>
              {t.tempoDesc[language]}
            </p>
          </div>

          <div className={`flex-1 min-h-[300px] rounded-3xl border shadow-inner overflow-hidden relative group transition-colors ${theme === 'dark' ? 'bg-[#1A1030] border-[#251848]' : 'bg-white border-gray-100'}`}>
            <div className="absolute inset-0 flex items-center justify-center gap-12 lg:gap-20">
              ...
              <div 
                ...
                className={`w-32 h-32 lg:w-40 lg:h-40 rounded-full shadow-2xl animate-pulse flex items-center justify-center transition-all ${isRecording ? 'bg-red-500 shadow-red-500/30 scale-110' : 'bg-gradient-to-tr from-primary to-[#B87AFF] shadow-primary/30'}`}
              >
              ...
              <div 
                ...
                className={`w-20 h-20 lg:w-28 lg:h-28 rounded-xl shadow-lg animate-bounce flex items-center justify-center ${theme === 'dark' ? 'bg-[#251848]' : 'bg-[#E8DCFF]'}`}
              >
            ...
            <div className={`absolute top-6 right-6 flex items-center gap-3 backdrop-blur-md px-4 py-2 rounded-full border shadow-sm ${theme === 'dark' ? 'bg-[#251848]/80 border-[#251848]' : 'bg-white/80 border-gray-200'}`}>
              ...
              <span className={`font-black tabular-nums ${theme === 'dark' ? 'text-[#E8DCFF]' : 'text-[#1A1A1A]'}`}>{bpm} BPM</span>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className={`flex items-center gap-4 p-4 rounded-2xl border transition-colors ${theme === 'dark' ? 'bg-[#1A1030] border-[#251848]' : 'bg-white border-gray-100'}`}>
              <div className="p-3 rounded-full bg-primary/10 text-primary">
                <span className="material-symbols-outlined">info</span>
              </div>
               <p className={`font-medium ${theme === 'dark' ? 'text-[#9B7EC8]' : 'text-gray-600'}`}>
            ...
            <div className={`flex items-center gap-4 p-4 rounded-2xl border transition-colors ${theme === 'dark' ? 'bg-[#1A1030] border-[#251848]' : 'bg-white border-gray-100'}`}>
              <div className={`p-3 rounded-full ${isRecording ? 'bg-red-500/20 text-red-500 animate-pulse' : 'bg-primary/10 text-primary'}`}>
                <span className="material-symbols-outlined">{isRecording ? 'recording' : 'settings_voice'}</span>
              </div>
              <p className={`font-medium text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
        ...
        <div className={`w-full lg:w-[400px] border-l p-6 flex flex-col justify-between gap-6 shadow-2xl transition-colors overflow-y-auto ${theme === 'dark' ? 'bg-[#1A1030] border-[#251848]' : 'bg-white border-slate-100'}`}>
          <div className="text-center space-y-1 mt-4">
            ...
            <div className={`text-6xl font-black tracking-tighter tabular-nums leading-none ${theme === 'dark' ? 'text-[#E8DCFF]' : 'text-[#1A1A1A]'}`}>
              {bpm} <span className="text-lg text-gray-300 font-bold align-top mt-2 inline-block">BPM</span>
            </div>
          </div>

          <div className="space-y-6">
            <div className="relative pt-2">
                <div className="absolute top-0 left-0 w-full flex justify-between text-[10px] font-black text-gray-400 uppercase tracking-widest">
                ...
                <input 
                    ...
                    className="w-full h-3 bg-yellow-300/80 dark:bg-white/10 rounded-full accent-primary appearance-none cursor-pointer mt-2"
                />
            </div>

            {/* Metronome Toggle */}
            <div className={`flex items-center justify-between p-3 rounded-xl border transition-colors ${theme === 'dark' ? 'bg-[#0F0A1A] border-[#251848]' : 'bg-slate-50 border-slate-100'}`}>
               <div className="flex items-center gap-2">
                  <span className={`material-symbols-outlined text-lg ${isTickEnabled ? 'text-primary' : 'text-gray-400'}`}>
                    {isTickEnabled ? 'notifications_active' : 'notifications_off'}
                  </span>
                  <span className={`font-bold text-sm ${theme === 'dark' ? 'text-[#E8DCFF]' : 'text-[#1A1A1A]'}`}>{t.metronomeSound[language]}</span>
               </div>
               <label className="relative flex cursor-pointer items-center">
                  ...
                  <div className="h-5 w-9 rounded-full bg-gray-300 peer-checked:bg-primary transition-colors after:absolute after:left-[2px] after:top-[2px] after:h-4 after:w-4 after:rounded-full after:bg-white after:transition-all peer-checked:after:translate-x-full"></div>
               </label>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <button 
                    ...
                    className={`flex flex-col items-center gap-1.5 p-3 rounded-xl transition-all ${label.name === item.name ? 'bg-primary/10 ring-2 ring-primary' : theme === 'dark' ? 'hover:bg-white/5' : 'hover:bg-gray-50'}`}
                >
                  <span className={`material-symbols-outlined text-2xl ${label.name === item.name ? 'text-primary' : 'text-gray-300'}`}>{item.icon}</span>
                  <span className={`font-black text-sm ${theme === 'dark' ? 'text-[#E8DCFF]' : 'text-[#1A1A1A]'}`}>{item.name}</span>
          ...
          <div className={`p-4 rounded-2xl border flex flex-col gap-3 transition-colors ${theme === 'dark' ? 'bg-[#0F0A1A] border-[#251848]' : 'bg-slate-50 border-slate-200'}`}>
            <h3 className={`font-black text-sm uppercase tracking-widest flex items-center gap-2 ${theme === 'dark' ? 'text-[#E8DCFF]' : 'text-[#1A1A1A]'}`}>
              <span className="material-symbols-outlined text-primary text-base">mic_none</span>
              {t.voiceExp[language]}
            </h3>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 px-2 rounded-lg text-sm font-bold transition-all shadow-sm active:scale-95 ${isRecording ? 'bg-red-500 text-white animate-pulse' : theme === 'dark' ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-white text-slate-700 hover:bg-gray-100'}`}
              >
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 px-2 rounded-lg text-sm font-bold transition-all shadow-sm active:scale-95 disabled:opacity-30 disabled:cursor-not-allowed ${isVoicePlaying ? 'bg-primary text-white' : theme === 'dark' ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-white text-slate-700 hover:bg-gray-100'}`}
              >
                <span className="material-symbols-outlined text-sm">{isVoicePlaying ? 'stop' : 'play_arrow'}</span>
